...
async function browserDriveSync(request, env) {
  ...
  if (user.role === 'viewer') return json({ ok:false, error:'Viewer accounts are read-only' },403);
  if (!driveSyncConfigured(env)) return json({ ok:false, error:'Google Drive synchronization is not configured' },503);
  try {
...
async function armLiveProof(request,env,platform,settingKey){
  const user=await currentUser(env,request);
  if(!user)return Response.redirect(new URL('/',request.url),302);
  ...
  const post=await env.DB.prepare(`SELECT sp.id,sp.caption,sp.scheduled_for,sp.status,p.name product_name,a.public_token,a.original_name
    ...
    WHERE lower(sp.platform)=?
      AND sp.status IN ('scheduled','approved')
      AND sp.scheduled_for>?
      AND lower(COALESCE(p.name,'')) LIKE '%buddy%'
    ORDER BY sp.scheduled_for ASC LIMIT 1`).bind(platform,new Date().toISOString()).first();
...
async function liveProofStatus(request,env,platform){
  const user=await currentUser(env,request);
  if(!user)return Response.redirect(new URL('/',request.url),302);
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (request.method === 'GET' && url.pathname === '/system/live-proof-pinterest') return armLiveProof(request,env,'pinterest','live_proof_pinterest');
    if (request.method === 'GET' && url.pathname === '/system/live-proof-pinterest-status') return liveProofStatus(request,env,'pinterest');
    if (request.method === 'GET' && url.pathname === '/system/live-proof-pinterest-diagnostic') return pinterestDiagnostic(env);
    if (request.method === 'GET' && url.pathname === '/system/live-proof-instagram') return armLiveProof(request,env,'instagram','live_proof_instagram');
    if (request.method === 'GET' && url.pathname === '/system/live-proof-instagram-status') return liveProofStatus(request,env,'instagram');
