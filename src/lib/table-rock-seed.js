...
export async function ensureTableRockPressSeed(env){
  ...
  for(let i=0;i<BUDDY_COPY.length;i++){
    ...
    const found=await env.DB.prepare(`SELECT id FROM copy_items WHERE id=?`).bind(cid).first();
    if(found) continue;
    const t=new Date().toISOString();
    await env.DB.prepare(`INSERT INTO copy_items(id,product_id,copy_type,text,audience,purpose,tone,length_class,campaign_type,status,source,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)`)
      .bind(cid,PRODUCT_ID,'caption',BUDDY_COPY[i],'Children and families','engagement','warm','medium','story','approved',SOURCE,t,t).run();
    inserted++;
  }
  return {product_id:PRODUCT_ID,copy_items:BUDDY_COPY.length,inserted};
}
